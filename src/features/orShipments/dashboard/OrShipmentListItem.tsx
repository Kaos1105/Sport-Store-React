...
const OrShipmentListItem: React.FC<{ shipment: IShipment; index: number }> = ({
  shipment,
  index,
}) => {
  const rootStore = useContext(RootStoreContext);
  const { deleteShipment, submitting, targetDelete } = rootStore.orderShipmentStore;
    ...
    <Table.Row key={shipment.id}>
      ...
      <Table.Cell textAlign='center'>
        <Button.Group>
          ...
          <Button // onClick={() => selectActivity(activity.id)}
            as={Link}
            to={`/orderShipment/${shipment.id}/manage`}
            color='orange'
          >
            <Icon name='edit' />{' '}
          </Button>
...
export default observer(OrShipmentListItem);
